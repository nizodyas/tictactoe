...
class Game(models.Model):
    ...
    def new_move(self):
        if self.status not in 'FS':
            raise ValueError("Cannot make in finished game")
        return Move(
            game=self, by_first_player=self.status == 'F'
        )

    def update_after_move(self, move):
        # update the status of game given last move
    ...
    def _get_game_status_after_move(self, move):
        ...
        if self.move_set.count() >= BOARD_SIZE:
            return 'D'
